#!/usr/bin/env python3
"""Pivot parsed mlip single-point results and calculate CC-reference errors.

Inputs
------
``data/intermediate/mlip_spe_results.csv``
    Long-form parsed output with energies in Hartree and GPU calculation times
    in seconds.
``configs/aliases.csv``
...
PARSED_RESULTS = PROJECT_ROOT / "data/intermediate/mlip_spe_results.csv"
ALIASES = PROJECT_ROOT / "configs/aliases.csv"
REFERENCE_DATA = PROJECT_ROOT / "data/intermediate/reference_data.csv"
PIVOT_OUTPUT = PROJECT_ROOT / "data/intermediate/mlip_species_pivot.csv"
REACTION_OUTPUT = PROJECT_ROOT / "data/final/mlip_reaction_results.csv"
TIME_INPUT_FIELD = "gpu_time_seconds"
TIME_OUTPUT_LABEL = "gpu_time"
...
def main() -> None:
    ...
    for reaction in sorted(reference_rows, key=int):
        for functional, program in function_programs:
            ...
            for state in STATES:
                ...
                calculation_time = number_or_none(record.get(TIME_INPUT_FIELD)) if record else None
                pivot[f"{state}_energy_kcal_mol"] = formatted(
                    energy_hartree * HARTREE_TO_KCAL_MOL if energy_hartree is not None else None
                )
                pivot[f"{state}_{TIME_OUTPUT_LABEL}_s"] = formatted(calculation_time)
    ...
    pivot_fields += [f"{state}_{TIME_OUTPUT_LABEL}_s" for state in STATES]
