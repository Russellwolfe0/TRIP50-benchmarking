#!/usr/bin/env python3
"""Creates a compact mlip benchmark table with reaction quantities and errors.

Run ``pivot_mlip_results.py`` first. This script uses its alias-corrected
...
PIVOT_RESULTS = PROJECT_ROOT / "data/intermediate/mlip_species_pivot.csv"
REFERENCE_DATA = PROJECT_ROOT / "data/intermediate/reference_data.csv"
OUTPUT_CSV = PROJECT_ROOT / "data/final/mlip_benchmark_results.csv"
TIME_INPUT_LABEL = "gpu_time"
TIME_OUTPUT_FIELD = "average_gpu_time_seconds"
...
def main() -> None:
    ...
    with PIVOT_RESULTS.open(newline="", encoding="utf-8") as stream:
        for row in csv.DictReader(stream):
            ...
            calculation_times = {
                state: number_or_none(row.get(f"{state}_{TIME_INPUT_LABEL}_s"))
                for state in ("R1", "R2", "TS", "P1", "P2")
            }
            ...
            available_times = [calculation_times[state] for state in required if calculation_times[state] is not None]
            ...
            results.append(
                {
                    ...
                    TIME_OUTPUT_FIELD: formatted(average_calculation_time),
